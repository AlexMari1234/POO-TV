...
public class FilterByActors implements FilterMovies {

    /**
     * Searches for movies in a given list based on a list of names of actors
    ...
    public ArrayList<Movies> searchMovies(final ArrayList<Movies> inputMovies,
        ...
        for (int i = 0; i < inputMovies.size(); i++) {
            if (inputMovies.get(i).getActors() != null) {
                for (int j = 0; j < conditionData.size(); j++) {
                    if (inputMovies.get(i).getActors().contains(conditionData.get(j))) {
